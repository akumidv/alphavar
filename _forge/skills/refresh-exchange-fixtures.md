...
`_forge/tools/exchange_fixtures/<exchange>.py`.
...
1. **Record** (needs network) — captures full responses keyed by path+query, with HTTP
   ...
   ```bash
   uv run python -m _forge.tools.exchange_fixtures            # both, or
   uv run python -m _forge.tools.exchange_fixtures --only deribit
   ```
