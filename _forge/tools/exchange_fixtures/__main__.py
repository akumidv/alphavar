...
path+query and stored with their HTTP status (so 4xx error paths are exercised too).
    ...
    uv run python -m _forge.tools.exchange_fixtures            # all exchanges
    uv run python -m _forge.tools.exchange_fixtures --only moex
...
Full playbook (when/why/verify): _forge/skills/refresh-exchange-fixtures.md.
"""
import argparse

from _forge.tools.exchange_fixtures import deribit, moex
