...
- [`exchange_fixtures/`](exchange_fixtures/) (code; docs in its `__main__` docstring,
  run `python -m _forge.tools.exchange_fixtures`): record live exchange API responses
...
- [`data_migration/`](data_migration/) (code; docs in its `__main__` docstring, run
  `python -m _forge.tools.data_migration`): the general **verify + fix** script for stored
