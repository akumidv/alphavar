...
def _legacy_df():
    return pd.DataFrame({
        ...
        "exchange_underlying_symbol": ["SYN.BTC-10APR25", "SYN.BTC-10APR25"],
        "kind": ["o", "o"],
        "option_type": ["c", "p"],
        "exchange_price": [0.1, 0.2],
        "exchange_iv": [50.0, 55.0],
...
def test_column_renames():
    ...
    for old in ("symbol", "exchange_symbol", "kind", "option_type",
                "exchange_price", "original_timestamp", "source_last"):
        assert old not in out.columns, old
    for new in ("asset_code", "exch_symbol", "instrument_kind", "option_right",
                "exch_price", "exch_timestamp", "last_raw"):
...
def test_value_remap_codes_to_readable():
    ...
    assert sorted(out["option_right"].unique()) == ["call", "put"]
