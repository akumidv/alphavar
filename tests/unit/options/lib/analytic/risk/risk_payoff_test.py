...
from alphavar.options.lib.analytic.risk import RiskColumns as RCl
...
def test__calc_profile_long_call(mock_df_op_chain, strike, tmp_output_dir):
    ...
    assert df_payoff[df_payoff[OptionsTerm.STRIKE] == strike_sell].iloc[0][RCl.RISK_PNL.nm] == expected_profit_call
...
def test_mock__chain_leg_payoff_put_itm(mock_df_op_chain, strike):
    ...
    assert df_payoff[df_payoff[OptionsTerm.STRIKE] == strike_sell].iloc[0][RCl.RISK_PNL.nm] == expected_profit_put
...
def test_mock__chain_leg_payoff_future(mock_df_op_chain):
    ...
    assert df_payoff[df_payoff[OptionsTerm.STRIKE] == strike_sell].iloc[0][RCl.RISK_PNL.nm] == expected_profit_fut
...
def test_chain_pnl_risk_profile_structure_long_straddle(df_chain, structure_long_straddle):
    ...
    df_leg1 = df_legs_payoff[df_legs_payoff[RCl.LEG_ID.nm] == legs_ids[0]][
        [OptionsTerm.STRIKE, RCl.RISK_PNL.nm]
