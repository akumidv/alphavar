...
COLUMN_RENAMES: dict[str, str] = {
    ...
    "exchange_underlying_symbol": Term.UNDERLYING_CODE,
    # classification (R4.5)
    ...
    "underlying_asset_type": Term.UNDERLYING_ASSET_CLASS,
    # price/iv + timestamp (R4.2)
    "exchange_price": Term.EXCH_PRICE,
    "exchange_iv": Term.EXCH_IV,
    "exchange_mark_price": Term.EXCH_MARK_PRICE,
    "exhchange_mark_price": Term.EXCH_MARK_PRICE,  # historical double typo
    "exchange_mark_iv": Term.EXCH_MARK_IV,
...
_TIMESTAMP_COLS = (
    Term.TIMESTAMP,
    Term.REQUEST_TIMESTAMP,
    Term.EXCH_TIMESTAMP,
    Term.EXPIRATION_DATE,
    Term.UNDERLYING_EXPIRATION_DATE,
)
...
def rename_legacy_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    (``exchange_mark_price``/typo → ``exch_mark_price``, ``original_timestamp`` →
    ``exch_timestamp``, ``source_<c>`` → ``<c>_raw``, …). **Idempotent** — a no-op on an
    already-current frame (renames fire only when the old name is present and the new one
    absent). Used on parquet load so stored files read with current names without running
    the full migration; value remaps / type coercion stay in ``migrate_dataframe``."""
