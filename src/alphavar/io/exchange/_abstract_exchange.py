...
class RequestClass:
    ...
    def request_api(self, endpoint_path: str, signed: bool = False, **kwargs):
        ...
        if signed:
            raise NotImplementedError(
                "Signed (authenticated) requests are not supported; only public endpoints are available."
            )
    ...
    def _request(self, request_url, **kwargs):
        ...
        raise RequestException(
            f"Request failed after {self.MAX_RETRIES} retries: {type(last_retryable).__name__}"
        ) from last_retryable
...
class AbstractExchange(AbstractProvider, ABC):
    ...
    # The pre-currency-conversion value of column ``<col>`` is stored as ``<col>_raw``
    # (R4.2 group 3 — was the ``source_<col>`` prefix; T23.6).
    RAW_SUFFIX = "_raw"
