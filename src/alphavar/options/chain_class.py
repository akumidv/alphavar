...
from alphavar.options.lib.chain.price_status import (
...
class OptionsChain:
    ...
    def select_chain(
        self, settlement_date: pd.Timestamp | None = None, expiation_date: pd.Timestamp | None = None
    ) -> pd.DataFrame:
    ...
    def get_settlement_longest_period_expired_date(self, settlement_date: pd.Timestamp | None = None) -> pd.Timestamp:
    ...
    def add_atm_itm_otm(self) -> Self:
        ...
        self._data.df_chain[OptionsTerm.PRICE_STATUS] = get_chain_atm_itm_otm(self._data.df_chain)
        return self
