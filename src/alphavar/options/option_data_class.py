...
from alphavar.options.dictionary import Timeframe, OptionsColumns as OCl
...
class OptionData:
    ...
    def __init__(
        ...
        self._opt_columns = (
            option_columns
            if isinstance(option_columns, list)
            else AbstractProvider.options_columns
        )
        self._fut_columns = (
            future_columns
            if isinstance(future_columns, list)
            else AbstractProvider.futures_columns
        )
    ...
    def df_hist(self) -> pd.DataFrame:
        ...
        if self._df_hist is None:
            ...
            self._df_hist = self._provider.load_options_history(
                self._option_symbol, params=opt_params, columns=self._opt_columns
            )
    ...
    def df_fut(self) -> DataFrame:
        ...
        if self._df_fut is None:
            ...
            self._df_fut = self._provider.load_futures_history(
                self._option_symbol, params=fut_params, columns=self._fut_columns
            )
        return self._df_fut
    ...
    def update_option_chain(
        ...
        df_chain: DataFrame | None = self._provider.load_options_chain(
            self.option_symbol, settlement_date, expiration_date
        )
        if df_chain is None:
            return False
