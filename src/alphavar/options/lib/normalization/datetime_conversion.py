...
def parse_expiration_date(date_str: str) -> datetime.date | None:
    ...
    try:
        ...
        logger.error("parsing expiration date %s: %s", date_str, err)
        return None
...
def df_columns_to_timestamp(df: pd.DataFrame, columns: list, unit: str | None = None) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    row = df.iloc[0]
    converting_columns = [col for col in columns if col in df.columns and not isinstance(row[col], pd.Timestamp)]
