...
> in-repo `src/alphavar/options/` (entities, enrichment, chain).
...
- **Premium = intrinsic value + time value.**
  ...
  - In-repo: `options/lib/enrichment/price.py` (`add_intrinsic_and_time_value`).
- **Moneyness:** ITM / ATM / OTM (call ITM when S>K; put ITM when S<K). In-repo:
  `options/lib/chain/price_status.py`, ATM-strike selection.
- **Option chain / desk:** grid of strikes × expirations for an underlying; the "desk"
  view pairs calls/puts per strike. In-repo: `options/lib/chain/`.
