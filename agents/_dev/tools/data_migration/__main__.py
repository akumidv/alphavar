...
Run (dry by default; the data path is an *exchange* folder, its sub-folders are asset codes):
    ...
    uv run python -m agents._dev.tools.data_migration verify /data/DERIBIT

    # 2) fix — column conversion (any *.parquet) + reference meta (per asset), then re-verify
    uv run python -m agents._dev.tools.data_migration fix /data/DERIBIT            # DRY (shows plan)
    uv run python -m agents._dev.tools.data_migration fix /data/DERIBIT --apply    # writes (.bak kept)
...
``agents/_dev/skills/migrate-stored-data.md``.
"""
...
from agents._dev.tools.data_migration._diagnose import Issue, diagnose_exchange
...
def _fix(exchange_dir: str, *, apply: bool) -> int:
    ...
    n_cols = migrate_parquet_tree(exchange_dir, apply=apply)
