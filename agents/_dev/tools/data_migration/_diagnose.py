...
from alphavar.core.migration.legacy_parquet import COLUMN_RENAMES
...
def diagnose_parquet(path: str) -> list[Issue]:
    ...
    legacy = [c for c in cols if c in COLUMN_RENAMES or c.startswith(_SOURCE_PREFIX)]
    if legacy:
        issues.append(Issue("error", "structure", path, f"legacy column names → run fix: {legacy}"))
    unknown = [c for c in cols if not _is_known(c) and c not in COLUMN_RENAMES and not c.startswith(_SOURCE_PREFIX)]
    if unknown:
