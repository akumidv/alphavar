...
`agents/_dev/tools/data_migration` (`python -m agents._dev.tools.data_migration`) drives the
...
1. **Verify first — see exactly what is wrong (metadata / structure / types):**
   ```bash
   uv run python -m agents._dev.tools.data_migration verify {root}/{EXCHANGE}
   ```
2. **Fix history (dry run, then apply):**
   ```bash
   uv run python -m agents._dev.tools.data_migration fix {root}/{EXCHANGE}           # DRY (plan)
   uv run python -m agents._dev.tools.data_migration fix {root}/{EXCHANGE} --apply   # writes + re-verifies
   ```
