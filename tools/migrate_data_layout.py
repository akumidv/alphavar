...
def validate_layout(root: str) -> None:
    ...
    if bad:
        raise LayoutError(
            "unrecognized kind directories (old layout — convert manually):\n  - " + "\n  - ".join(sorted(bad))
        )
...
def plan_renames(root: str) -> list[tuple[str, str]]:
    ...
    for dirpath, dirnames, _ in os.walk(root, topdown=False):
        for name in dirnames:
            if name in RENAME:
                renames.append((os.path.join(dirpath, name), os.path.join(dirpath, RENAME[name])))
    return renames
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--contents", action="store_true", help="also expand parquet column values via legacy_parquet migrator"
    )
    parser.add_argument(
        "--no-validate",
        action="store_true",
